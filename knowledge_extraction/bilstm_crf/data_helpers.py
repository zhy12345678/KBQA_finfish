...
class NerDataProcessor(object):
    ...
    def read_data(self,path,is_training_data=True):  #如果是训练集就是Ture不是训练集就是False 如果是验证集或者测试集就没有必要去弄这个了
        """
        数据格式如下（分隔符为空格）：
        便 B_disease
        秘 I_disease
        ...
        sentence = []
        labels = []
        split_pattern = re.compile(r'[；;。，、？！\.\?,! ]')   #对每一个句子进行切分，尽量在有标点符号的位置上做切分
        with open(path,'r',encoding = 'utf8') as f:
            for line in f.readlines():
                #每行为一个字符和其tag，中间用tab或者空格隔开
                line = line.strip().split()
                if(not line or len(line) < 2):    #如果是空行就会将他作为样本
